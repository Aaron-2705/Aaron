...
// Adapted from React Bits <LightPillar /> (three.js, WebGL). Ported to TS and
// tuned to the steel theme (blue accent, not the upstream purple/pink). An
// ethereal volumetric light pillar for the hero backdrop. Reduced-motion is
// handled by the caller (pass rotationSpeed=0 + interactive=false to freeze).
...
export default function LightPillar({
  ...
  const [webGLSupported, setWebGLSupported] = useState<boolean>(detectWebGL);
  ...
  useEffect(() => {
    ...
    const qualitySettings: Record<"low" | "medium" | "high", QualitySetting> = {
      low: { iterations: 24, waveIterations: 1, pixelRatio: 0.5, precision: "mediump", stepMultiplier: 1.5 },
      medium: { iterations: 40, waveIterations: 2, pixelRatio: 0.65, precision: "mediump", stepMultiplier: 1.2 },
      high: {
        iterations: 80,
        waveIterations: 4,
        // Capped at 1.5x (not 2x): the pillar is a soft glow, so rendering at
        // full retina density quadruples the raymarch cost for no visible gain.
    ...
    try {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- WebGL context creation can throw on some GPUs; degrade to the fallback UI.
    ...
    // Pause the (expensive raymarch) render whenever the pillar is scrolled
    // off-screen. Without this it keeps drawing at 60fps behind the rest of the
    // page's WebGL scenes and saturates the GPU. Matches the off-screen-pause
    // pattern used by the other 3D surfaces in this project.
    let visible = true;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- the scene is rebuilt only on WebGL support or quality change; live prop updates are handled by the effects below.
  ...
  if (!webGLSupported) {
    return (
      <div className={`light-pillar-fallback ${className}`} style={{ mixBlendMode }}>
        {/* Static graceful fallback when WebGL is unavailable. */}
      </div>
    );
  }
